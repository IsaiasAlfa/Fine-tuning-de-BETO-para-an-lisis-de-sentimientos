...
def cargar_dataset():
  ...
  datos = dataset.to_pandas()
  datos = datos[["text", "gold"]].rename(columns={"gold": "label"})
...
def obtener_dataloaders(tokenizer):
  ...
  dataset_train = BenchDataset(textos_train, labels_train, tokenizer)
  dataset_val = BenchDataset(textos_val, labels_val, tokenizer)
